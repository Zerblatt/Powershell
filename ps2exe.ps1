Param([string]$inputFile=$null, [string]$outputFile=$null, [switch]$verbose, [switch] $debug, [switch]$runtime20, [switch]$runtime40,
	...
	[switch]$requireAdmin, [switch]$virtualize)
...
if (!$nested)
{
	Write-Host "PS2EXE-GUI v0.5.0.10 by Ingo Karstein, reworked and GUI support by Markus Scholtes"
}
else
{
...
if ([string]::IsNullOrEmpty($inputFile) -or [string]::IsNullOrEmpty($outputFile))
{
	...
	Write-Host "               [-verbose] [-debug] [-runtime20|-runtime40] [-lcid <id>] [-x86|-x64] [-Sta|-Mta]"
